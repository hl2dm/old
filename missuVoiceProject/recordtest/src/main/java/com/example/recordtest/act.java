        ...
        public void handleMessage(Message msg) {
            ...
            switch(vCmd)
            {
                case CMD_RECORDING_TIME:
                    ...
                    act.this.txt.setText("正在录音中，已录制："+vTime+" s");
                    break;
                case CMD_RECORDFAIL:
                    int vErrorCode = b.getInt("msg");
                    String vMsg = ErrorCode.getErrorInfo(act.this, vErrorCode);
                    act.this.txt.setText("录音失败："+vMsg);
                    break;
                case CMD_STOP:
                    ...
                    switch(vFileType){
                        case FLAG_WAV:
                            ...
                            act.this.txt.setText("录音已停止.录音文件:"+AudioFileFunc.getWavFilePath()+"\n文件大小："+mSize);
                            break;
                        case FLAG_AMR:
                            MediaRecordFunc mRecord_2 = MediaRecordFunc.getInstance();
                            mSize = mRecord_2.getRecordFileSize();
                            act.this.txt.setText("录音已停止.录音文件:"+AudioFileFunc.getAMRFilePath()+"\n文件大小："+mSize);
                            break;
                    }
                    break;
                default:
    ...
    class UIThread implements Runnable {
        ...
        public void run() {
            while(vRun){
                ...
                act.this.uiHandler.sendMessage(msg); // 向Handler发送消息,更新UI
            }
